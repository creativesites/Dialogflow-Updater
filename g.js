...
puppeteer.launch({ headless: false }).then(async (browser) => {
  ...
  if (headless) {
    ...
    try {
        await page.click('#yDmH0d > c-wiz.yip5uc.SSPGKf > c-wiz > div > div.p9lFnc > div > div > div > div.ZRg0lb.Kn8Efe > div:nth-child(3) > div > div.yKBrKe > div');  
    } catch (error) {
        
    }
   
  }
});//select menu button if needed
/* await page1.waitForTimeout(4500);
await page1.waitForSelector('#inner-header > div > button > span.flaticon.stroke.menu-3',{ timeout: 0 });
await page1.waitForTimeout(2500);
console.log('selecting copy button')
await page1.click('#inner-header > div > button > span.flaticon.stroke.menu-3') */